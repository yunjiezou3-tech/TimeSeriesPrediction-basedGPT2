...
class Dataset_TSF_ICL(Dataset):
    def __init__(self, root_path, flag='train', size=None, data_path=None,
        ...
        self.pred_len = size[2] # 预测步长 = 模型要预测的未来时间步
        self.token_len = self.pred_len # 输入片段长度，to be predicted？
        self.context_len = 4 * self.token_len # include：prompt + target
    ...
    def __read_data__(self):
        ...
        def dropna(x):
            return x[~np.isnan(x)]
        timeseries = [dropna(ts).astype(np.float32) for ts in df.series_value]
        timeseries = [ts for ts in timeseries if ts.shape[0] > self.context_len] # 要求timeseries长度大于context_len，从而从中抽取两段，分别作为“prompt”和“（目标序列）to be predicted”
        return timeseries

    # we uniformly adopting the first time points of the time series as the corresponding prompt.
    # 我们统一采用时间序列的首次出现时间点作为对应的提示词。
    def __getitem__(self, index):
            # [--------------------------------800 timesteps--------------------------------]
            # |<----Prompt zone---->|                     |<-----Target zone------->|
            # |      0~200          |        ...          | 600~700 | 700~800       |
            # nlp 视角
            # Prompt: "Here are previous examples of this time series: [t0 ... t199]."
            # Context: "The recent pattern is: [t600 ... t699]."
            # Question: "What happens next?"
            # → Model
            # generates[t700...t799]
        data_x1 = self.timeseries[index][:2 * self.token_len]  # prompt part
        data_x2 = self.timeseries[index][-2 * self.token_len:-1 * self.token_len]  # last history part（目标序列的历史部分）
        data_x = np.concatenate((data_x1, data_x2))  # 拼接 prompt + lookback
        data_y = self.timeseries[index][-1 * self.token_len:]  # prediction target（目标序列的未来部分）
        data_x = np.expand_dims(data_x, axis=-1)
        data_y = np.expand_dims(data_y, axis=-1)
        return data_x, data_y, data_x, data_y
